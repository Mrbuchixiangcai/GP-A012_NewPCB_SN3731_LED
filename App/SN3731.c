...
//static unsigned char g_SN3731_IIC_ADDRESS = SN3731_IIC_ADDRESS_M; //改为了宏定义
...
//其他动画
unsigned int  g_fireworks_NO;	//用于记录烟花动画播放到第几帧
unsigned int  g_fireworks_Temp;
unsigned char g_lighting_NO;	//用于记录mode1动画播放到第几帧
unsigned char g_unlock_NO;		//用于记录mode2动画播放到第几帧
unsigned char g_water_NO;		//用于记录mode3动画播放到第几帧
unsigned char g_jingtaitu_NO;	//用于记录mode4动画播放到第几帧
unsigned char g_audio1_NO;		//用于记录mode4 picture 播放到第几帧
		...
		if (g_MovieEnd_Flag)   //判断动画是否为结束的尾帧，如果是则播放完该动画的尾帧，然后再重新开始播放，形成循环
		{
			if (g_rail != 0)   //轨道，电极，机匣
			{
				...
				if (g_times & 0x01)//判断为奇偶数，此处为当前动画总帧数为4的奇数倍，偶数与上0x01 bit0一定为1
				{
					...
					SN3731_WriteRegs(0x00, all_led_off, 18);	   				//因为SN3731在播放结束时，图片是停在最后一帧的下一帧
					SN3731_WriteRegs(0x24, all_pwm_off, 144);					//而如果不清零的话，很可能在movie的末尾与下一动画衔接时会闪烁一下。
				}
				else//当前动画的总帧数为4的偶数倍
				{
					...
					SN3731_WriteRegs(0x00, all_led_off, 18);	   				//如果尾数不为零，且当前动画的总帧数为4的偶数倍则为数帧清零
