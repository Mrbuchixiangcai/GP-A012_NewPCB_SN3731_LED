...
void BlueMode_Receive(void)
{
	...
	if (Uart1_RX_Timeout != 0)
	{
		if (--Uart1_RX_Timeout == 0)
		{
			...
			if(BT_ReceiveBuffer[0] == '\0')//按下power接收到的‘\0’，
			{
			...
			else if ((BT_ReceiveBuffer[0] == 'I') && (BT_ReceiveBuffer[1] == 'I'))//开蓝牙
			{
				PlayMode=PLAY_BT;
			}
			else if ((BT_ReceiveBuffer[0] == 'I') && (BT_ReceiveBuffer[1] == 'A'))//关蓝牙，规定要关闭所有功能，所以要关机
			{
				PlayMode=PLAY_OFF;
			}
			ptrReceive = 0;
